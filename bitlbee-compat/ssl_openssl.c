...
static gboolean ssl_handshake_cb(gpointer data, gint source, b_input_condition cond)
{
	gboolean rv = ssl_handshake(data, source, cond);
...
static gboolean ssl_handshake(gpointer data, gint source, b_input_condition cond)
{
	...
	if ((st = SSL_connect(conn->ssl)) < 0) {
		...
		conn->inpa = b_input_add(conn->fd, ssl_getdirection(conn), ssl_handshake_cb, data);
		return FALSE;
	}
